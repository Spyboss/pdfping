...
app.post("/api/v1/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  ...
  if (!sig) return res.status(400).json({ error: "Missing x-signature header" });
  ...
  const apiKey = data.attributes?.first_order_item?.product_options?.custom?.api_key ||
  ...
  let plan = "free";
  let limit = 10;
  if (variantId === process.env.LEMONSQUEEZY_PRO_VARIANT_ID) { plan = "pro"; limit = 500; }

  if (["order_created", "subscription_created", "subscription_updated"].includes(eventName)) {
    if (apiKey) {
      if (supabase) {
        await supabase.from("api_keys").update({ plan, limit_count: limit }).eq("key", apiKey);
  ...
  if (["subscription_cancelled", "subscription_expired"].includes(eventName)) {
    if (apiKey) {
      if (supabase) {
        await supabase.from("api_keys").update({ plan: "free", limit_count: 10 }).eq("key", apiKey);
      } else if (apiKeys.has(apiKey)) {
        const k = apiKeys.get(apiKey);
        k.plan = "free";
        k.limit = 10;
...
async function authenticate(req, res, next) {
  ...
  if (supabase) {
    ...
    if (!key) {
      return res.status(403).json({ error: "Invalid API key. Get one at https://pdfapi.uhadev.com" });
    }
