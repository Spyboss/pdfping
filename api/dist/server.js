...
async function withBrowserPage(fn) {
    ...
    try {
        ...
        metrics.renderCount++;
        if (elapsed < metrics.minRenderTimeMs)
            metrics.minRenderTimeMs = elapsed;
        if (elapsed > metrics.maxRenderTimeMs)
            metrics.maxRenderTimeMs = elapsed;
        return result;
    }
    finally {
        await page.close();
...
async function renderPdf(html, url, options = {}) {
    const { format = "A4", landscape = false, printBackground = true, wait = 1, margin = "0mm", } = options;
...
app.post("/api/v1/convert", authenticate, async (req, res) => {
    ...
    try {
        ...
        if (metrics.recentFailures.length > 100)
...
app.post("/api/v1/convert/public", publicConvertLimiter, async (req, res) => {
    const { html, url, options = {} } = req.body;
    if (!html && !url) {
...
app.get("/api/v1/metrics", authenticate, async (req, res) => {
    const avgRender = metrics.renderCount > 0
    ...
    res.json({
        ...
        minRenderTimeMs: metrics.minRenderTimeMs === Infinity ? 0 : metrics.minRenderTimeMs,
